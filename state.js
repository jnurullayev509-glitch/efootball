...
async function readState() {
  const { blobs } = await list({ prefix: PATH, limit: 10, token: process.env.BLOB_READ_WRITE_TOKEN });
  const blob = blobs.find(b => b.pathname === PATH);
  if (!blob) return { players: {}, results: {} };
  const r = await fetch(blob.url, { cache: 'no-store' });
  if (!r.ok) return { players: {}, results: {} };
  return await r.json();
}
...
export default async function handler(req, res) {
  try {
    ...
    if (!admins.includes(String(user.id))) return res.status(403).json({ error: 'Admin only' });
    ...
    const blob = await put(PATH, JSON.stringify(state), {
      access: 'public',
